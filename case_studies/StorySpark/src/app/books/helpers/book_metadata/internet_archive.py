...
class InternetArchiveProvider:
    def fetch(self, isbns: list) -> dict[str, list[str]]:
        ...
        editions_filtered = InternetArchiveProvider.get_editions(isbns)
        ...
        for edition in editions_filtered:
            metadata_for_edition = InternetArchiveProvider.get_metadata_for_edition(edition)
            metadata.extend(metadata_for_edition)

        return sorted(set(metadata))
    
    def get_editions(isbns:list) -> list[str]:
        query = " OR ".join(f"isbn:{isbn}" for isbn in isbns)
        params = {
            "q": f"({query})",
            "fl[]": "identifier",
            "rows": 100,
